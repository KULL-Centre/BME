...
def gamma(lambdas):
    ...
    sum1 = np.dot(lambdas,[avgx_true,avgy_true])
...
exp_sigma = np.array([0.4,0.5])

avgx_0 = np.average(x_0)
avgy_0 = np.average(y_0)

print "# Experimental average  x=%5.3f y=%5.3f" % (avgx_true,avgy_true)
...
print "chix %5.3f chiy %5.3f" % (np.abs((avg_post[0]-avgx_true))/exp_sigma[0],np.abs((avg_post[1]-avgy_true))/exp_sigma[1])
...
plt.savefig("fig02C.pdf",figsize=600)
plt.close()
...
axs[0].axis('off')
#
rows = ["True",'Prior',r"$\theta=4$",r"$\theta=0$"]
columns= ["<x>","<y>","S1 (%)","S2 (%)","S3 (%)"]
cell_text = [["%4.2f" % avgx_true,"%4.2f" % avgy_true, "%4.1f" % (pop1_true*100.),"%4.1f" % (pop2_true*100.),"%4.1f" % (pop3_true*100.)],\
             ["%4.2f" % avgx_0,"%4.2f" % avgy_0, "%4.1f" % (pop1_0*100.),"%4.1f" % (pop2_0*100.),"%4.1f" % (pop3_0*100.)],\
             ["%4.2f" % avg_post[0],"%4.2f" % avg_post[1], "%4.1f" % (pop1_post*100.),"%4.1f" % (pop2_post*100.), "%4.1f" % (pop3_post*100.)],\
             ["%4.2f" % 4.8,"%4.2f" % 5.4, "%4.1f" % 21.4,"%4.1f" % 31.1, "%4.1f" % 47.5]]

colors = ["#cec7c7",cols2[-3],cols3[-4],"#ffffff"]
rowColours=colors
