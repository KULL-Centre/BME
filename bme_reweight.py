...
def chi_square(exp_data, sim_data, weights, bounds=None):
    ...
    if bounds is None:
        ...
        idxs = [
            0.0
            if (
                (diff[i] >= 0 and bounds[i][1] == 0)
                or (diff[i] <= 0 and bounds[i][0] == 0)
            )
            else 1.0
...
def read_exp(filename):
    ...
    assert first[0] == "#", (
        "Error. First line of exp file %s must be in the format \# DATA=XXX PRIOR=XXX"  # noqa: E501
        % filename
    )
    ...
    assert (
        data_type in exp_types
    ), "Error. DATA must be one of the following: %s " % (exp_types)

    assert (
        prior_type in prior_types
    ), "Error. PRIOR must be one of the following: %s " % (prior_types)
    ...
    if len(first) == 4 and first[3] == "POWER":
        ...
        assert noe_power > 0.0, "# Error. POWER has to be larger than 0"
...
def weight_exp(exp_file, sim_file, w0, w_opt, outfile, rows=[], cols=[]):
    ...
    if len(rows) == 0:
        rows = range(sim_data.shape[0])
    if len(cols) == 0:
        cols = range(sim_data.shape[1])
    ...
    assert len(w_opt) == len(sim_data), (
        "# Number of weights %d not equal to number of data from simulation (%d)"  # noqa: E501
        % (len(w_opt), len(sim_data))
    )
    ...
    assert len(labels) == len(sim_data[0]), (
        "# Number of rows in %s not equal to number of columns in %s"
        % (len(labels), len(sim_data[0]))
    )

    # before/after reweighting
    chisq_0 = chi_square(exp_data, L0 * sim_data, w0, bounds=bounds) / len(
        exp_data
    )
    bcalc_0 = back_calc(L0 * sim_data, w0)

    chisq_1 = chi_square(exp_data, L1 * sim_data, w_opt, bounds=bounds) / len(
        exp_data
    )
    ...
    for l in range(exp_data.shape[0]):
        ...
        fh_stats.write(
            "   %-15s %9.3e %9.3e %9.3e %9.3e "
            % (
                labels[l],
                exp_data[l, 0],
                np.sqrt(exp_data[l, 1]),
                bcalc_0[l],
                bcalc_1[l],
            )
        )
        if data_type == "NOE":
            exp_avg = np.power(exp_data[l, 0], -1.0 / noe_power)
            sim_0 = np.power(bcalc_0[l], -1.0 / noe_power)
            sim_1 = np.power(bcalc_1[l], -1.0 / noe_power)
            sigma = np.sqrt(exp_data[l, 1]) * (
                exp_avg / (noe_power * exp_data[l, 0])
            )
            ...
            if (diff_0 >= 0 and bounds[l][0] == 0) or (
                diff_0 <= 0 and bounds[l][1] == 0
            ):
                ff_0 = 0.0

            if (diff_1 >= 0 and bounds[l][0] == 0) or (
                diff_1 <= 0 and bounds[l][1] == 0
            ):
                ff_1 = 0.0
...
class Reweight:
    ...
    def __init__(self, verbose=True, w0=[], kbt=None):
        ...
        if len(w0) != 0:
            ...
            if kbt is not None:
                print(
                    "# Assuming weights given as minus free energies. w=exp(bias/kbt) kbt=%8.4f "  # noqa: E501
                    % kbt
                )
                w0 = np.exp((w0 - np.max(w0)) / kbt)
            print(
                "# Set non-uniform initial weights from file. Sum=",
                np.sum(w0),
                len(w0),
            )
    ...
    def add_data(self, exp_data, sim_data, labels, bounds, cols):
        ...
        if len(self.sim_data) == 0:
            ...
            self.exp_data = np.concatenate(
                (self.exp_data, np.array(exp_data)[cols, :]), axis=0
            )
    ...
    def load(self, exp_file, sim_file, rows=[], cols=[]):
        ...
        if len(rows) == 0:
            rows = range(sim_data1.shape[0])
        if len(cols) == 0:
            cols = range(sim_data1.shape[1])
        if len(self.w0) != 0 and self.renormalize_w0 is False:
        ...
        assert len(labels) == sim_data.shape[1], (
            "# Number of rows in exp (%d) not equal to number of columns in calc (%d)"  # noqa: E501
            % (len(labels), sim_data.shape[1])
        )
        ...
        for l in range(len(exp_data)):
            ...
            if bounds[l][0] == 0.0 and mins[l] > exp_data[l][0]:
                print(
                    "# Warning: expt boundary %20s=%-10.4e is smaller than minimum value in simulation %-10.4e"  # noqa: E501
                    % (labels[l], exp_data[l][0], mins[l])
                )
            if bounds[l][1] == 0.0 and maxs[l] < exp_data[l][0]:
                print(
                    "# Warning: expt boundary %20s=%-10.4e is larger than maximum value in simulation %-10.4e"  # noqa: E501
                    % (labels[l], exp_data[l][0], mins[l])
                )

            if exp_data[l][0] > maxs[l]:
                print(
                    "# Warning: expt average %20s=%-10.4e is larger than maximum value in simulation %-10.4e"  # noqa: E501
                    % (labels[l], exp_data[l][0], maxs[l])
                )
            if exp_data[l][0] < mins[l]:
                print(
                    "# Warning: expt average %20s=%-10.4e is smaller than minimum value in simulation %-10.4e"  # noqa: E501
                    % (labels[l], exp_data[l][0], mins[l])
                )
    ...
    def optimize(self, theta, method="MAXENT"):
        def func_maxent_gauss(lambdas):
            ...
            if zz < 1.0e-300:
                ...
                print(
                    "# Perhaps you cannot fit the data. You may try with a larger theta?"  # noqa: E501
                )
        ...
        def func_ber_gauss(w):
            ...
            idxs = [
                0.0
                if (
                    (diff[i] >= 0 and self.bounds[i][1] == 0)
                    or (diff[i] <= 0 and self.bounds[i][0] == 0)
                )
                else 1.0
            ...
            srel = self.theta * np.sum(
                w[idxs] * np.log(w[idxs] / self.w0[idxs])
            )
        ...
        chi_sq0 = chi_square(
            self.exp_data, self.sim_data, self.w0, self.bounds
        ) / len(self.exp_data)
        ...
        if method == "BER":
            ...
            bounds = [(0.0, 1.0)] * len(self.w0)
            meth = "SLSQP"
            print(
                "# Bayesian Ensemble Refinement. Useful for testing purposes and "  # noqa: E501
            )
            print(
                "# when the number of experimental data is larger than the number of samples."  # noqa: E501
            )
        ...
        if result.success:
            chi_sq1 = chi_square(
                self.exp_data, self.sim_data, w_opt, self.bounds
            ) / len(self.exp_data)
            self.w_opt = np.copy(w_opt)
            srel1 = srel(self.w0, w_opt)
            # print("# Minimization successful")
            # print("Final average chi squared   %10.4f, srel %10.4f " % (chi_sq1, srel(self.w0,w_opt)))  # noqa: E501

            return chi_sq0, chi_sq1, -srel1
        else:
            print(
                "# ERROR - Minimization failed. Perhaps theta is too small, or there is some bug."  # noqa: E501
            )
    ...
    def get_lambdas(self):
        assert (
            self.method == "MAXENT"
        ), "# Error. You can get lambdas only if method=MAXENT"
        assert (
            self.lambdas is not None
        ), "# Error. Minimization not successful, no lagrange multipliers"
        return self.lambdas
    ...
    def weight_exp(self, exp_file, sim_file, outfile, rows=[], cols=[]):
        return weight_exp(
            exp_file, sim_file, self.w0, self.w_opt, outfile, rows, cols
        )
...
"""
    def func_maxent_laplace(lambdas):
        ...
        # weights
        ww  = self.w0*np.exp(-np.sum(lambdas[np.newaxis,:]*self.sim_data,axis=1))  # noqa: E501
        # normalization
        zz = np.sum(ww)
        ww /= zz
        # new averages
