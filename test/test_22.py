...
num=22
spec="Train RDC, test NOE, rows, non-uniform-weights"
...
def test():
    ...
    # load data
    rew.load(exp_train,calc_train,rows=rows)
    ...
    chi2_b,chi2_a = rew.weight_exp(exp_train,calc_train, '%s/test_%02d_train' % (outdir,num),rows=rows)
    chi2_b,chi2_a = rew.weight_exp(exp_test,calc_test, '%s/test_%02d_test' % (outdir,num),rows=rows)
